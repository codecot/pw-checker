...
const CRITICAL_CATEGORIES = new Set([
  'banking', 'finance', 'investment', 'cryptocurrency', 'work', 'email',
  'government', 'healthcare', 'insurance', 'social', 'shopping', 'cloud'
]);
...
function categorizeAccount(name: string, uri?: string): string {
  ...
  // Banking and Finance
  if (lowerName.includes('bank') || lowerUri.includes('bank') ||
      lowerName.includes('paypal') || lowerUri.includes('paypal') ||
      lowerName.includes('stripe') || lowerUri.includes('stripe') ||
      lowerName.includes('credit') || lowerName.includes('loan')) {
    return 'banking';
  }

  // Work/Professional
  if (lowerName.includes('work') || lowerName.includes('office') ||
      lowerName.includes('company') || lowerName.includes('enterprise') ||
      lowerUri.includes('slack') || lowerUri.includes('teams') ||
      lowerUri.includes('zoom') || lowerUri.includes('jira')) {
    return 'work';
  }

  // Email
  if (lowerName.includes('email') || lowerName.includes('mail') ||
      lowerUri.includes('gmail') || lowerUri.includes('outlook') ||
      lowerUri.includes('yahoo') || lowerUri.includes('@')) {
    return 'email';
  }

  // Social Media
  if (lowerUri.includes('facebook') || lowerUri.includes('twitter') ||
      lowerUri.includes('instagram') || lowerUri.includes('linkedin') ||
      lowerUri.includes('tiktok') || lowerUri.includes('snapchat')) {
    return 'social';
  }

  // Cloud Services
  if (lowerUri.includes('google') || lowerUri.includes('microsoft') ||
      lowerUri.includes('apple') || lowerUri.includes('amazon') ||
      lowerUri.includes('dropbox') || lowerUri.includes('icloud')) {
    return 'cloud';
  }
  ...
  // Government
  if (lowerName.includes('gov') || lowerUri.includes('.gov') ||
      lowerName.includes('tax') || lowerName.includes('irs')) {
    return 'government';
  }

  // Shopping
  if (lowerUri.includes('amazon') || lowerUri.includes('ebay') ||
      lowerUri.includes('shop') || lowerName.includes('store')) {
    return 'shopping';
  }
...
async function getBitwardenSession(): Promise<string | null> {
  try {
    ...
    console.log(chalk.yellow('🔐 Bitwarden vault is locked. Please unlock it first:'));
...
export async function importFromBitwarden(): Promise<void> {
  ...
  try {
    ...
    console.error(chalk.red('❌ Bitwarden CLI not found. Please install it first:'));
  ...
  try {
    ...
    for (const item of loginItems) {
      try {
        ...
        console.error(chalk.red(`❌ Error importing item "${item.name}":`, error));
        skipped++;
      }
    }
    ...
    console.log(chalk.cyan(`   • Total processed: ${imported + skipped} entries`));

    if (imported > 0) {
      console.log(chalk.yellow('\n💡 Next steps:'));
      console.log(chalk.yellow('   • Run breach check: npm run check:breaches'));
      console.log(chalk.yellow('   • Calculate risk scores: npm run risk:calculate'));
      console.log(chalk.yellow('   • Generate security report: npm run risk:report'));
    }
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    ...
    console.log('');
    console.log('Usage:');
    console.log('  npm run import:bitwarden     Import passwords from Bitwarden');
    console.log('');
